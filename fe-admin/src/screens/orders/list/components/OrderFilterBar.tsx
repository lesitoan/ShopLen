...
export function OrderFilterBar({
  ...
  const debouncedSearch = useMemo(
    () => debounce((val: string) => onSearchChange(val), 1000),
    [onSearchChange]
  );
    ...
    <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-start flex-wrap gap-3 bg-surface p-3 rounded-lg border border-border">
      ...
      {onResetFilter && (
        <button
          ...
          className="h-[38px] px-3 rounded-md bg-surface-muted hover:bg-surface-hover text-text-secondary hover:text-text-primary border border-border transition-colors flex items-center gap-1.5 text-xs font-medium ml-auto cursor-pointer"
