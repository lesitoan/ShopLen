...
export default function CartItemList({
  ...
  const hasOutOfStockItem = items.some((item) => !item.isAvailable || item.stock <= 0);
          ...
          {items.map((item) => (
            <div
              ...
              className={`py-4 flex flex-col md:grid md:grid-cols-12 gap-4 items-center ${
                !item.isAvailable || item.stock <= 0 ? "opacity-75 bg-background/50 rounded-lg p-2 md:p-0" : ""
              }`}
            >
